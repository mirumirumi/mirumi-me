...
const _color = ref(p.color)
const viewBox = ref("0 0 512 512")

setViewBox()

onMounted(() => {
  _color.value = theme.value === "dark" ? (p.dark ?? p.color) : p.color
})

watch(theme, () => {
  if (theme.value === "dark") {
    _color.value = p.dark ?? p.color
  } else {
    _color.value = p.color
  }
})
