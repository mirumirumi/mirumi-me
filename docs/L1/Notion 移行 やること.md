    ...
    - 記事更新差分ビルド API ✅
        ...
        - 画像同期
            ...
            - S3 key は画像セット共通の `assetHash` と人が読める元ファイル名を結合する
                ...
                - 本文は `{assetHash}-{cleanStem}-{setWidth}x{setHeight}-{actualWidth}w.webp`、thumbnail は `{assetHash}-{cleanStem}-{width}x{height}.webp`
                    - `{setWidth}x{setHeight}` は最大 variant（fallback）の実寸で、同じ画像セットの全 variant が共有する。フロントエンドは URL だけで本文画像の `width` / `height` を出す（CLS と、`srcset` 付きの小さい画像が `sizes` の幅まで引き伸ばされるのを防ぐ）
                    - 新しい本文 animation も `{assetHash}-{cleanStem}-{width}x{height}.{元の拡張子}` にして寸法を持たせる
        ...
        - `更新日` も Workers がセットする
            - 再公開で本文・title・画像などの内容が前回の配信から変わったときだけ現在日時をセットする（publish index の `sourceHash` で判定、誤字修正でも動く）
            - full build は Notion へ書き戻さないので `更新日` も動かない
    ...
    - 目次リンクを見出し名にしたい
        ...
        - 設計：
            ...
            - ただしこの方式では Notion 移行しないと id がわからない以上、事前に既存記事のリンク先を更新をすることはできない
                ...
                - 置換スクリプトは本番コンテンツ移行の後、リリースの前に `bun run --cwd tools/migrate-to-notion fix-toc-anchors --apply` を実行する
